...
const ChangePassword = () => {
  ...
  const {
    ...
    mode: "onBlur",
    resolver: yupResolver(userSchema.changePassword),
  });
      ...
      .then((res) => {
        ...
        Swal.fire({
          ...
          preConfirm: async (value) => {
            await userApi
              .finishChangePassword({ userCode: value })
              .then(() => {})
              .catch((err) => {
          ...
          if (result.isDismissed)
            if (result.dismiss === "timer")
              Swal.fire({
                icon: "error",
                title: "Error",
                html: "<p>Mã xác thực đã hết hạn<p/>Vui lòng dùng đăng ký lại!",
          ...
          if (result.isConfirmed)
            Swal.fire({
              ...
              icon: "success",
              title: "SUCCESS",
              html: "<p>Mật khẩu đã thay đổi<p/>Bạn có muốn đến trang đăng nhập ?",
      ...
      .catch((err) => {
        if (err.status === 400)
          Swal.fire({
            icon: "error",
            title: "Đã xảy ra lỗi",
            html: "<p>Tài khoản email chưa được đăng ký<p/>Vui lòng kiểm tra lại email!",
            confirmButtonText: "Đồng ý",
            allowOutsideClick: false,
          });
      });
